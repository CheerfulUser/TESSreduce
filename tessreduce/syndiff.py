...
def Get_TESS_corners(TESS, PS1_wcs):
    ...
    for i in range(x):
        for j in range(y):
            # why is this offset by 1????
            corners[:, i, j] = pix2coord(x_arr[i], y_arr[j], TESS.wcs)
            ps_corners[:, i, j] = coord2pix(
                corners[0, i, j], corners[1, i, j], PS1_wcs)
...
def Get_PS1(RA, DEC, Size, filt='i'):
    ...
    Scale = 100
    # int((Size + 2*np.sqrt(1/2)*Size) * Scale ) # last term is a fudge factor
    size = Size * Scale
...
def PS1_scene(tpf, size=20, convolve='PS1', plot=False):
    ...
    if convolve.lower() == 'ps1':
        try:
            # Interp_PRF(tpf.row + (tpf.flux.value.shape[1]/2), tpf.column + (tpf.flux.value.shape[2]/2),
            kernal = Gaussian2DKernel(1.5*100)
            # tpf.camera, tpf.ccd, tpf.sector)
            ps1_image = signal.fftconvolve(ps1_image, kernal, mode='same')
        except MemoryError:
            raise MemoryError(
                "The convolution is too large, try a smaller PS1 image.")
    ...
    if plot:
        ...
        plt.subplot(1, 2, 1)
        plt.title('PS1 image')
        plt.imshow(ps1_image/np.nanmax(ps1_image),
                   origin='lower', vmax=.1, cmap='Greys')
        x, y = tpf.flux.value.shape[1:]
        x += 1
        y += 1
        z = np.arange(0, x*y, 1)
        plt.scatter(tess_corners[0, :, :].flatten(),
                    tess_corners[1, :, :].flatten(), c=z, s=1)

        plt.subplot(1, 2, 2)
        plt.title('TESS image')
        plt.imshow(tpf.flux[100]/np.nanmax(tpf.flux[100]),
                   origin='lower', vmax=.1)
    ...
    if plot:
        ...
        plt.subplot(1, 2, 1)
        plt.title('PS1 scene')
        plt.imshow(rotate(np.flipud(ps1_scene/np.nanmax(ps1_scene)
                                    ), -90), origin='lower', vmax=0.1)
        plt.subplot(1, 2, 2)
        plt.title('TESS image')
        plt.imshow(tess.flux[0]/np.nanmax(tess.flux[0]),
